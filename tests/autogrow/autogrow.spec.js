...
test.describe(`<${componentName}>`, () => {
	...
	test('default component', async ({ page }) => {
		...
		const height = await text.evaluate((elem) => Number.parseFloat(window.getComputedStyle(elem).height));

		// Textarea should expand with long text
		await text.fill(wizards);
		const longHeight = await text.evaluate((elem) => Number.parseFloat(window.getComputedStyle(elem).height));
		await expect(longHeight).toBeGreaterThan(height);

		// Textarea should shrink again with short text
		await text.fill('Merlin');
		const shortHeight = await text.evaluate((elem) => Number.parseFloat(window.getComputedStyle(elem).height));
